<?php
$q4 = "SELECT v.kode, v.sumcar, z.jumlah, z.sumtot, z.lul FROM (SELECT f.kode, sum(g.poin*f.freq) sumcar FROM (SELECT * FROM unit a LEFT JOIN (SELECT * FROM user b JOIN aktivitas_employee c ON b.iduser = c.id_user where role = 0) d ON a.kode = d.unit) f LEFT JOIN subaktivitas g ON f.id_subaktivitas = g.id_subaktivitas group by f.kode) v left join (select *, (jumlah*sumtot) lul from (select kode, count(*) jumlah from unit a LEFT join user b ON a.kode = b.unit  where role = 0 group by a.kode) a, (SELECT sum(poin*max_freq) sumtot from subaktivitas where default2 = 1) b) z ON v.kode = z.kode";
$r4 = $db->query($q4);
if (!$r4) {
